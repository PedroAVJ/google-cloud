...
gcloud storage buckets describe gs://<bucket> \
  --project=<project> \
  --format="value(name,location,uniformBucketLevelAccess.enabled,publicAccessPrevention)"
...
`publicAccessPrevention: inherited` only means public access is not forcibly
blocked; it does not prove the bucket is public. Confirm the IAM policy grants
the intended read role to `allUsers`, then verify the exact object URL without
credentials. Never place anything private, personal, or client-confidential in
a bucket exposed to `allUsers`.
